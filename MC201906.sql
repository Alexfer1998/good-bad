...
s_rev_atm, saldo_lp, b.saldo_mora, saldo_exceso, dias_atraso as DiasMora201906,
...
flg_usodisef, zona_apein
 From mae_titular_uba_201906 a inner join tmpPeriodo b on b.id_cuenta = a.idencuenta
), tmpRcc as
(
...
sum(ImporteSoles) as [ConsumoAnual201806_201906],
count(ImporteSoles) as [NroTransaccionAnual201806_201906]
FROM mtransaccion_uba A --a join VTRANS  b on a.IdenCuenta = b.IdenCuenta
where   periodotx >= 201806 and periodotx <201906 AND DSPTIPOTRX = 'Establecimientos'
...
ConsumoAnual201806_201906,NroTransaccionAnual201806_201906, tipo_seguro, 
isnull(c.[201812],-1) as [HabitoPago201812], isnull(c.[201901],-1) as [HabitoPago201901],isnull(c.[201902], -1) as [HabitoPago201902], 
isnull(c.[201903], -1) as [HabitoPago201903], isnull(c.[201906],-1) as [HabitoPago201906], isnull(c.[201907],-1) as [HabitoPago201907],
...
INTO ModeloCliente201906
From tmptotal06 a left join tmprcc b on (a.numdoc = b.nrodocumento)
...
where flg_mayor = 1 and diasmora201906 < 8


-----------------------------------------------------------
UPDATE ModeloCliente201906 SET Departamento = 'Lima'
WHERE DEPARTAMENTO= ''
;
UPDATE ModeloCliente201906 SET ESTADOCIVIL= 'SOLTERO'
WHERE ESTADOCIVIL= ''
;
--------------------------------------------------------
ALTER TABLE ModeloCliente201906 ALTER COLUMN HABITOPAGO201906 VARCHAR(10);
ALTER TABLE ModeloCliente201906 ALTER COLUMN HABITOPAGO201905 VARCHAR(10);
ALTER TABLE ModeloCliente201906 ALTER COLUMN HABITOPAGO201904 VARCHAR(10);
ALTER TABLE ModeloCliente201906 ALTER COLUMN HABITOPAGO201903 VARCHAR(10);
ALTER TABLE ModeloCliente201906 ALTER COLUMN HABITOPAGO201902 VARCHAR(10);
ALTER TABLE ModeloCliente201906 ALTER COLUMN HABITOPAGO201901 VARCHAR(10)
;
...
select a.*, b.dias_Atraso as DiasAtraso3m, c.dias_atraso as DiasAtraso6m 
 From (ModeloCliente201906 a left join tmpMora3m b on a.idencuenta = b.id_cuenta)
	left join tmpMora6m c on  a.idencuenta = c.id_cuenta
WHERE idencuenta not in 
(select idencuenta 
 FRom ModeloCliente201906
 where HabitoPago201905 = '-1'  and HabitoPago201904 = '-1'
 and HabitoPago201901 = '-1' and HabitoPago201902 = '-1' and HabitoPago201906 = '-1'
 and HabitoPago201903 = '-1'  )
 ) SELECT *, CASE WHEN HABITOPAGO201906 IN ('1','0') OR  HABITOPAGO201905 IN ('1', '0') OR HABITOPAGO201904 IN ('1','0') 
	THEN '0'
	else  '1'
	END [FlagHabitoPago3M],
 CASE WHEN  HABITOPAGO201902 IN ('1','0') OR  HABITOPAGO201905 IN ('1', '0') OR HABITOPAGO201903 IN ('1','0') 
	or  HABITOPAGO201906 IN ('1','0') OR  HABITOPAGO201901 IN ('1', '0') OR HABITOPAGO201904 IN ('1','0') 
	THEN '0'
	else  '1'
	END [FlagHabitoPago6M],case when diasatraso3m > 8 or diasAtraso6m > 8 then 'MAL CLIENTE'
